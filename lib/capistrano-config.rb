      ...
      configuration.load {
        namespace(:config) {
          ...
          def update_all(files={}, options={})
            ...
            tmps = files.map { capture("t=$(mktemp /tmp/capistrano-config.XXXXXXXXXX);rm -f $t;echo $t").chomp }
          ...
          def update_all_locally(files={}, options={})
            srcs = files.map { |src, dst| src }
            tmps = files.map { `t=$(mktemp /tmp/capistrano-config.XXXXXXXXXX);rm -f $t;echo $t`.chomp }
            dsts = files.map { |src, dst| dst }
            begin
              srcs.zip(tmps).each do |src, tmp|
